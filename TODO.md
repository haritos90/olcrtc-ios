...
| 432 | logs | Shared logs were unidentifiable (bare text → "text.txt", current tab only) and background behaviour was invisible — a background gap couldn't be told apart from a real suspension | Exports now write a NAMED file (`olcrtc-<log>-v<ver>-b<build>-<ts>.log`) with a header block (log name, app version+build, device/OS, line count, capture window); added **Export all logs** (every category + per-server container buffer in one file) and header-prefixed Copy (`LogExport`, `.shareFileLazy`). Background: `LogStore.flush()` fsyncs on background entry; `BackgroundRuntimeKeeper` recovers from AVAudioSession interruptions + media-services resets; a per-minute background heartbeat plus background/foreground lifecycle lines in the connection log make a suspension visible instead of guessed. 19 unit tests | Shared logs now carry a descriptive filename and a version/device header, there's an "Export all logs" option, and the connection log shows clear background keep-alive/heartbeat markers — so a log is never a mystery and you can see if it kept recording in the background |
| 433 | reliability | Pinned olcrtc core was 83 commits behind upstream master — a large in-framework reliability payload (carrier disconnects, multi-client, jitsi ICE, teardown stalls) sat unshipped | Moved the `olcrtc-upstream` submodule pin 9822def → 42ae4e0, rebuilt `App/Mobile.xcframework` via `build-framework.sh`, and absorbed the single `srv.sh` upstream drift (wbstream `auth.token`) as a `# boc olcrtc-ios` patch sourcing `OLCRTC_WB_TOKEN`; `parity_check.py` green (267 adopted / 469 accounted). Mobile API is additive only (`SetWBToken`); URI wire contract unchanged | Updated the tunnel core to the latest upstream — carrier-reconnect, multi-client and faster-teardown reliability fixes |
| 434 | parity | CarrierTransportMatrix marked jitsi seichannel/videochannel as fail, stale vs upstream master | Master's E2E `case "jitsi"` returns ExpectPass for every transport (RTP keepalive fixes landed for sei/video), so jitsi seichannel + videochannel flip `.fail` → `.ok`; matrix + `testMatrixMatchesUpstream` updated | The compatibility matrix now shows Jitsi working with every transport |
| 436 | features | No way to supply a wbstream account token, so wbstream-over-datachannel (which needs publish rights) was unusable | Added a per-connection `wbToken`, Keychain-only like `key`/`socksPass` (never serialised — Codable-excluded, hydrated on load): masked field in the install sheet, `MobileSetWBToken` before Start, `OLCRTC_WB_TOKEN` → `srv.sh` `auth.token`, plus `redactSecrets` scrubbing and installEnv/redaction/Codable-exclusion tests | You can set a wbstream account token when installing a server, enabling wbstream over datachannel |
| 437 | automation | No way to connect/disconnect the tunnel from Shortcuts, the Action button, or Siri | Added in-binary `ConnectTunnelIntent` / `DisconnectTunnelIntent` + an `AppShortcutsProvider`, reaching the live engine via new `TunnelManager.shared` / `ConnectionStore.shared` weak refs (no extension target, so sideload re-signing is unaffected) | Connect or disconnect the tunnel from Shortcuts, the Action button, or Siri |
